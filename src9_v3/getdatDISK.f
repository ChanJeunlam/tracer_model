...
c
      ...
      integer          iexpt,yrflag,i1,i2,j1,j2
c
...
c
      ...
      deallocate (drec)
c
c     Read smoothed archives (climatology) if desired
c     (use "mean" instead of a year, and "ann" instead of the day, if
c     annual means are used)
c
      if (timflg.eq.3) then
c
c     Open the file with smoothed time-dependent fields
c
         print*,'READ data from ',flnm(1:l)
         open(ni+1001,file=trim(flnm),access='direct',
     $        recl=nrecl,form='unformatted',status='old')
      endif
c
      if (timflg.ge.2) write(flnm(l-4:l-2),'(a3)') 'ann'
c
      print*,'READ data from ',flnm(1:l)
      open(ni+1000,file=trim(flnm),access='direct',
     $     recl=nrecl,form='unformatted',status='old')
c
c --- read instantaneous dp
c
      do k=1,kk
         read(ni+1000,rec=k)((dpio(i,j,k),i=i1,i2),j=j1,j2)
         if(k.eq.1.or.k.eq.25) then
            write(6,*) itest,jtest,'dpio=',dpio(itest,jtest,k)
         endif
      enddo
c
...
c
      ...
      do k=1,kk
         ...
         read(nf,     rec=recsh+4) (( difs(i,j,k),i=i1,i2),j=j1,j2)
         if (ghtflg.eq.1) then
            read(nf,  rec=recsh+5) ((ghats(i,j,k),i=i1,i2),j=j1,j2)
         endif
      enddo                     !k
c
c --- read mixed layer fields (they can be different from the rest)
c
      if(mxlflg.eq.1) then
         print*,'READ MLD from ',flnm
         recsh=kk+nforf*kk
         read(nf,     rec=recsh+1) ((dpmixl(i,j),i=i1,i2),j=j1,j2) 
      endif
      return
c
