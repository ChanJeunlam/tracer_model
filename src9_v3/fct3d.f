...
c
c  tracer    - transported mixing ratio, e.g., salinity or temperature
c  u,v,w  - mass fluxes (x time step) satisfying continuity equation
c           (diaflx(k) = mass flux per unit area across bottome of layer k)
...
c
      do 23 k=1,kk-1
      slab=onemu
      if (diaflx(i,j,k).lt.0.) then ! interface moves down
...
c> Oct. 2020 - revised to eliminate eddy transport
c>
